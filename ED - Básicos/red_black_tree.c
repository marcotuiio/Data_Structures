...
void insertTree(Tree t, int i) {
    ...
    Red_Black_Node *new = insertAux(t, red_black_tree->root, i);
    fixTree(t, red_black_tree->root, new);
}
...
Node insertAux(Tree t, Node n, int i) {
    ...
    if (!new_node) {
        ...
        if (!red_black_tree->root) {
            ...
            strcpy(red_black_tree->root->color, "BLACK");
        }
        if (new_node) {
            red_black_tree->size++;
            return new_node;
    ...
    while ((new != root) && (!isBlack(new)) && (isRed(new->parent))) {
        ...
        if (parent_new == grandpa_new->left) {
            ...
            if (uncle_new && isRed(uncle_new)) {
                strcpy(grandpa_new->color, "RED");
                strcpy(parent_new->color, "BLACK");
                strcpy(uncle_new->color, "BLACK");
                new = grandpa_new;
            
            } else {
                // CASO 2A: new é filho a direita de seu pai, precisa rotacionar para a esquerda
...
void rotateLeft(Tree t, Node n) {
    ...
    Red_Black_Node *aux2 = right_child->left;
